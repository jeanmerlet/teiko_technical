...
The loading, analysis, and dashboard components are kept separate so that the database and all outputs can be reproduced without starting the dashboard. The complete analysis is contained in analysis.py and organized into modular functions. For a larger or more varied analysis, these functions could be divided into more specialized scripts, but that additional structure was unnecessary for this focused assessment. Database tables are loaded into pandas DataFrames for analysis because pandas provides a compact and readable interface for these analyses, while SQL is used directly where database-side filtering / aggregation are most appropriate.
